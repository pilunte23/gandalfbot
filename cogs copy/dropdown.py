...
class mydropdown(nextcord.ui.Select):
    def __init__(self):
        ...
        selectoptions = [
            nextcord.SelectOption(label="test1",description="desctest1",value ="1"),
            nextcord.SelectOption(label="test2",description="desctest2",value="2"),
            nextcord.SelectOption(label="test3",description="desctest3",value="3")
        ]
        
        drop_object = super().__init__(placeholder="Quelle carte voulez vous afficher? ",min_values=1,max_values=1 ,options=selectoptions)
        

    async def callback(self, interaction: nextcord.Interaction):
        word = self.values[0]
        return await interaction.response.send_message(f"Choix: {word}")
...
class Drop(commands.Cog):
    ...
    @nextcord.slash_command(description="Test l'affichage d'un dropdown",guild_ids=list(map(int,str(os.getenv("GUILDID")).split(" "))))

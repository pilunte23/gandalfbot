...
class Phase(commands.Cog):
    ...
    @nextcord.slash_command(name="phase",description="Affiche le timing des phases dans Seigneur des anneaux JCE",guild_ids=list(map(int,str(os.getenv("GUILDID")).split(" "))))
    async def _timing(self, interaction: Interaction, timing: str = nextcord.SlashOption(name="timing", choices=["phases", "combat"])):
        print(f"{interaction.user} use Phase slash command" )

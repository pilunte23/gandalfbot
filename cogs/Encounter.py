...
class Encounter(commands.Cog):
    ...
    type: str = SlashOption(name="type",description="Filtre sur le Type de Carte (Héro, Allié, Attachement, Evènement...)",required=False,choices=["Pas de filtre (par défaut)","Ennemi","Lieu","Traitrise","Objectif","Quête annexe rencontre","Préparation"]),
    ...
    terme: str = SlashOption(name="terme",description="Terme Exacte ou Partiel",required=False, choices=["Terme partiel (par défaut)", "Terme exact"])):
        print(f"{interaction.user} use Encounter slash command" )
